#!/usr/bin/env python3
"""
Load Detailnetz (Berlin Detailed Road Network) data into PostGIS
Includes: Road Segments, Connection Points, and Engineering Structures
"""
...
SCHEMA = 'vector'
TABLE_MAPPINGS = {
    'berlin_detailnetz_road_segments': 'detailnetz_road_segments',
    'berlin_detailnetz_connection_points': 'detailnetz_connection_points',
    'berlin_detailnetz_structures': 'detailnetz_structures'
}
...
def main():
    """Main function"""
    print("=" * 80)
    print("LOADING DETAILNETZ (BERLIN DETAILED ROAD NETWORK) DATA INTO POSTGIS")
    print("=" * 80)
    ...
    input_dir = project_root / "data/vector/detailnetz"

    # Find all detailnetz GeoJSON files
    input_files = glob.glob(str(input_dir / "berlin_detailnetz_*.geojson"))

    if not input_files:
        print(f"❌ No input files found in {input_dir}")
        print(f"\nFirst run: python scripts/download_detailnetz.py")
        return False
    ...
    for input_file in sorted(input_files):
        ...
        table_name = TABLE_MAPPINGS.get(filename, filename.replace('berlin_detailnetz_', 'detailnetz_'))
    ...
    print(f"\n{'=' * 80}")
    print("✅ DETAILNETZ DATA SUCCESSFULLY LOADED")
    print(f"{'=' * 80}")
    ...
    print(f"    'Show main roads in Berlin'")
    print(f"    'Find all bridges in the city'")
    print(f"    'Which roads pass near hospitals?'")
    print(f"    'Display the street network'")
    print()
