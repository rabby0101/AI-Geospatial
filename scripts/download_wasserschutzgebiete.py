#!/usr/bin/env python3
"""
Download Wasserschutzgebiete (Water Protection Areas) from Berlin's official WFS service
...
# WFS endpoint
WFS_BASE_URL = "https://gdi.berlin.de/services/wfs/wsg"
...
def main():
    """Download Wasserschutzgebiete data from Berlin WFS"""

    print("=" * 80)
    print("DOWNLOADING WASSERSCHUTZGEBIETE (WATER PROTECTION AREAS) FROM BERLIN GDI WFS")
    print("=" * 80)
    print(f"\nSource: {WFS_BASE_URL}")
    ...
    # Look for Wasserschutzgebiete layers
    wsg_layers = [l for l in layers if 'wsg' in l.lower()]

    if not wsg_layers:
        print(f"\n⚠️  No Wasserschutzgebiete layers found. Available layers: {layers}")
        print("\nTrying first available layer...")
        wsg_layers = layers[:1]

    print(f"\nWasserschutzgebiete layers found: {wsg_layers}\n")

    all_gdfs = []

    for layer in wsg_layers:
    ...
    if not all_gdfs:
        print("\n❌ Failed to download any Wasserschutzgebiete data")
        return False
    ...
    # Save to file
    output_file = output_dir / "berlin_wasserschutzgebiete.geojson"
    ...
    print(f"   python scripts/load_wasserschutzgebiete.py\n")
