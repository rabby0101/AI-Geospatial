#!/usr/bin/env python3
"""
Download Bauleitplanung (Building Plans/Land Use Planning) data from Berlin's official WFS service
Source: https://gdi.berlin.de/services/wfs/plu_bplan
Features: SpatialPlan and OfficialDocumentation
"""
...
# WFS endpoint
WFS_BASE_URL = "https://gdi.berlin.de/services/wfs/plu_bplan"
    ...
    try:
        ...
        for version in ['2.0.0', '1.1.0', '1.0.0']:
            try:
                ...
                if bbox and version in ['2.0.0', '1.1.0']:
                    if version == '2.0.0':
                        params['BBOX'] = f"{bbox[0]},{bbox[1]},{bbox[2]},{bbox[3]},urn:ogc:def:crs:EPSG:4326"
                    else:
...
def main():
    """Download Bauleitplanung data from Berlin WFS"""

    print("=" * 80)
    print("DOWNLOADING BAULEITPLANUNG (BUILDING PLANS/LAND USE) FROM BERLIN GDI WFS")
    print("=" * 80)
    print(f"\nSource: {WFS_BASE_URL}")
    print(f"Timestamp: {datetime.now().isoformat()}\n")

    project_root = Path(__file__).parent.parent
    output_dir = project_root / "data/vector/wfs"
    ...
    for layer in layers:
        ...
        gdf = download_wfs_layer(layer, bbox=BERLIN_BBOX)
    ...
    if not all_gdfs:
        print("\n❌ Failed to download any Bauleitplanung data")
        return False
    ...
    for layer_name, gdf in all_gdfs.items():
        ...
        # Create filename from layer name
        safe_name = layer_name.split(':')[-1].lower()
        output_file = output_dir / f"berlin_bplan_{safe_name}.geojson"

        print(f"\nSaving {layer_name} to {output_file}...", end="", flush=True)
    ...
    print(f"   python scripts/load_bplan_landuse.py\n")
