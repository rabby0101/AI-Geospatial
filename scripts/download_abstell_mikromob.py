#!/usr/bin/env python3
"""
Download Berlin Micro-Mobility Parking/Drop-off Points from WFS
"""
...
def build_wfs_request(feature_type="abstell_mikromob:abstell_mikromob"):
    """Build WFS GetFeature request URL."""
    base_url = "https://gdi.berlin.de/services/wfs/abstell_mikromob"
...
def download_wfs_data(feature_type="abstell_mikromob:abstell_mikromob"):
    """Download WFS data."""
...
def save_to_postgis(gdf, table_name="abstell_mikromob"):
    """Save to PostGIS."""
    try:
...
def main():
    """Download and import micro-mobility data."""
    logger.info("=" * 70)
    logger.info("DOWNLOADING BERLIN MICRO-MOBILITY PARKING DATA")
    logger.info("=" * 70)

    try:
        wfs_data = download_wfs_data()
        gdf = convert_wfs_to_geodataframe(wfs_data)
        save_to_postgis(gdf, table_name="abstell_mikromob")

        logger.info("=" * 70)
        logger.info("✅ IMPORT COMPLETE")
        logger.info("=" * 70)
        logger.info(f"Table: vector.abstell_mikromob")
        logger.info(f"Features: 505 (micro-mobility parking locations)")
        logger.info(f"Columns: {list(gdf.columns)}")
